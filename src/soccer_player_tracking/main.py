...
def main():
    ...
    print(
        f"GK x range:  {positions[0, :, 0].min():.2f} – {positions[0, :, 0].max():.2f}"
    )
    print(
        f"ST x range:  {positions[8, :, 0].min():.2f} – {positions[8, :, 0].max():.2f}"
    )
    ...
    for i, player in enumerate(team.players):
        ...
        print(
            f"{player.name:<5}  raw: {rmse_raw:.2f}m  filtered: {rmse_filt:.2f}m  "
